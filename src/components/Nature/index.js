import p5 from "p5"
import hash from "hash.js"
import uuid from "uuid/v4"

import Entity from "components/Entity"

export default class Nature {
  constructor(p, props) {
    const { seed = uuid(), chunkSize = 4096 } = props

    this.p = p

    this.id = hash
      .sha256()
      .update(seed)
      .digest("hex")

    this.chunks = []
    this.chunkSize = chunkSize

    console.log(`Generated Nature with id ${this.id}`)
  }

  update(x, y, z) {
    const { chunkSize } = this
    for (let dx = -1; dx <= 1; dx += 1) {
      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dz = -1; dz <= 1; dz += 1) {
          this.spawnChunkIfNew(
            x + dx * chunkSize,
            y + dy * chunkSize,
            z + dz * chunkSize
          )
        }
      }
    }
  }

  spawnChunkIfNew(x, y, z) {
    const { p, chunks, chunkSize } = this

    const thisChunkX = Math.floor(x / chunkSize)
    const thisChunkY = Math.floor(y / chunkSize)
    const thisChunkZ = Math.floor(z / chunkSize)

    const thisChunkPos = p.createVector(thisChunkX, thisChunkY, thisChunkZ)

    let isNew = true
    for (const chunk of chunks) {
      if (chunk.getPos().dist(thisChunkPos) === 0) {
        isNew = false
      }
    }

    if (isNew) {
      chunks.push(new Chunk(p, { pos: thisChunkPos, size: chunkSize }))
    }
  }

  render(x, y, z) {
    this.update(x, y, z)

    const { chunks } = this

    for (const chunk of chunks) {
      chunk.render()
    }
  }
}

class Chunk {
  constructor(p, props) {
    const { pos = p.createVector(0, 0, 0), size = 2048, seed = uuid() } = props

    this.p = p
    this.pos = pos
    this.center = p5.Vector.mult(pos, size)
    this.size = size

    this.id = hash
      .sha256()
      .update(seed)
      .digest("hex")

    this.clusters = []
    this.entities = []

    this.setup()

    console.log(`Generated new chunk with id ${this.id}`)
    console.log(this)
  }

  setup() {
    const { p, center, size } = this

    const weights = []
    const numWeights = 16
    for (let i = 0; i < numWeights; i += 1) {
      weights[i] = idHash(this.id, Math.floor((i / numWeights) * 20))
    }

    const density = weights[0] * 0.005
    const numClusters = density * size
    for (let i = 0; i < numClusters; i += 1) {
      const cx = p.map(
        Math.random(),
        0,
        1,
        center.x - size / 2,
        center.x + size / 2
      )
      const cy = p.map(
        Math.random(),
        0,
        1,
        center.y - size / 2,
        center.y + size / 2
      )
      const cz = p.map(
        Math.random(),
        0,
        1,
        center.z - size / 2,
        center.z + size / 2
      )

      if (Math.random() > weights[1]) {
        // cluster
        this.clusters.push(new Cluster(p, { pos: p.createVector(cx, cy, cz) }))
      } else {
        // individual entity
        this.entities.push(
          new Entity(p, {
            pos: p.createVector(cx, cy, cz)
          })
        )
      }
    }
  }

  render() {
    const { entities, clusters } = this

    for (const entity of entities) {
      entity.render()
    }

    for (const cluster of clusters) {
      cluster.render()
    }
  }

  getPos() {
    return this.pos
  }
}

class Cluster {
  constructor(p, props) {
    const { pos, numEntities = 10, spread = 100 } = props

    this.p = p
    this.pos = pos
    this.spread = spread

    this.numEntities = numEntities
    this.entities = []

    this.setup()
  }

  setup() {
    const { p, pos, spread, numEntities } = this

    for (let i = 0; i < numEntities; i += 1) {
      const cx = p.map(
        Math.random(),
        0,
        1,
        pos.x - spread / 2,
        pos.x + spread / 2
      )
      const cy = p.map(
        Math.random(),
        0,
        1,
        pos.y - spread / 2,
        pos.y + spread / 2
      )
      const cz = p.map(
        Math.random(),
        0,
        1,
        pos.z - spread / 2,
        pos.z + spread / 2
      )

      this.entities.push(
        new Entity(p, { size: 64, pos: p.createVector(cx, cy, cz) })
      )
    }
  }

  render() {
    const { entities } = this

    for (const entity of entities) {
      entity.render()
    }
  }
}

function idHash(id, val) {
  const pos = val % (id.length - 1)
  const length = 2

  return (
    parseInt((id + id).substring(pos, pos + length), 16) /
    (Math.pow(16, length) - 1)
  )
}
